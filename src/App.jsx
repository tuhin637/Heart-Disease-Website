...
const STEPS    = [
  { id:0, title:"Physical Metrics", icon:"⚡" },
  { id:1, title:"Lifestyle",        icon:"🏃" },
  { id:2, title:"Medical History",  icon:"🩺" },
  { id:3, title:"Demographics",     icon:"👤" },
];
const INIT = {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for(let i=0;i<pts.length;i++) for(let j=i+1;j<pts.length;j++) {
        ...
        if(d<100) { ctx.beginPath(); ctx.moveTo(pts[i].x,pts[i].y); ctx.lineTo(pts[j].x,pts[j].y); ctx.strokeStyle=`rgba(0,255,135,${(1-d/100)*0.08})`; ctx.lineWidth=0.5; ctx.stroke(); }
      }
...
function RiskMeter({ pct, level }) {
  ...
  useEffect(() => {
    ...
    [[0,25,"#00ff87"],[25,50,"#facc15"],[50,75,"#fb923c"],[75,100,"#ef4444"]].forEach(([s,e,c]) => {
      ctx.beginPath(); ctx.arc(cx,cy,R,Math.PI*.75+(s/100)*Math.PI*1.5,Math.PI*.75+(e/100)*Math.PI*1.5);
    ...
    if(drawn>0) {
      ...
      ctx.beginPath(); ctx.arc(cx,cy,R,Math.PI*.75,Math.PI*.75+(drawn/100)*Math.PI*1.5);
      ctx.strokeStyle=col; ctx.lineWidth=lw; ctx.lineCap="round";
      ctx.shadowBlur=20; ctx.shadowColor=col; ctx.stroke(); ctx.shadowBlur=0;
      const ang=Math.PI*.75+(drawn/100)*Math.PI*1.5;
      ctx.beginPath(); ctx.arc(cx+R*Math.cos(ang),cy+R*Math.sin(ang),lw/2+2,0,Math.PI*2);
    ...
    <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:32 }}>
      {STEPS.map((s,i) => (
        <div key={i} style={{ display:"flex", alignItems:"center", flex:1 }}>
          <div style={{ width:36, height:36, borderRadius:"50%", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, background:i<current?"linear-gradient(135deg,#00ff87,#00c9ff)":i===current?"rgba(0,255,135,0.15)":"rgba(255,255,255,0.04)", border:i===current?"1.5px solid #00ff87":i<current?"none":"1.5px solid rgba(255,255,255,0.08)", boxShadow:i===current?"0 0 16px rgba(0,255,135,0.3)":"none", transition:"all .4s" }}>
            {i<current?"✓":s.icon}
          </div>
          {i===current && (
            <div style={{ marginLeft:8 }}>
              <div style={{ fontSize:11, color:"#64748b", fontFamily:"'Space Mono',monospace", letterSpacing:1 }}>STEP {i+1}</div>
              <div style={{ fontSize:13, color:"#e2e8f0", fontWeight:600, fontFamily:"'Syne',sans-serif" }}>{s.title}</div>
            </div>
          )}
          {i<STEPS.length-1 && (
            <div style={{ flex:1, height:1, marginLeft:8, background:i<current-1?"linear-gradient(90deg,#00ff87,#00c9ff)":i===current-1?"linear-gradient(90deg,#00ff87,rgba(255,255,255,0.06))":"rgba(255,255,255,0.06)", transition:"background 0.5s" }} />
          )}
        </div>
      ))}
    </div>
...
const fld = {
  ...
  lbl:{ fontSize:13, fontWeight:500, color:"#94a3b8", fontFamily:"'Outfit',sans-serif" },
  inp:{ width:"100%", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:10, padding:"10px 14px", color:"#e2e8f0", fontSize:15, fontFamily:"'Space Mono',monospace", outline:"none", boxSizing:"border-box" },
...
function Toggle({ label, field, value, onChange }) {
  return (
    <div style={fld.wrap}>
      ...
      <div style={{ display:"flex", borderRadius:10, overflow:"hidden", border:"1px solid rgba(255,255,255,0.08)", width:"fit-content" }}>
        {["No","Yes"].map(opt => (
          <button key={opt} type="button" onClick={() => onChange(field, opt)} style={{ padding:"9px 28px", border:"none", cursor:"pointer", fontFamily:"'Outfit',sans-serif", fontSize:14, fontWeight:500, background:value===opt?(opt==="Yes"?"linear-gradient(135deg,#ef4444,#dc2626)":"linear-gradient(135deg,#00ff87,#00c9ff)"):"rgba(255,255,255,0.03)", color:value===opt?(opt==="Yes"?"#fff":"#0a0a0a"):"#64748b", transition:"all .25s" }}>{opt}</button>
        ))}
      </div>
    </div>
  );
...
function Num({ label, field, value, onChange, min, max, step="0.1", unit }) {
...
function RiskCard({ rf, index }) {
  ...
  return (
    <div style={{ padding:"16px 18px", borderRadius:12, marginBottom:10, background:impGlow[rf.impact], border:`1px solid ${col}28`, borderLeft:`3px solid ${col}`, opacity:vis?1:0, transform:vis?"translateX(0)":"translateX(-20px)", transition:"opacity .4s, transform .4s" }}>
    ...
    <div style={{ minHeight:"100vh", background:"#060910", color:"#e2e8f0", fontFamily:"'Outfit',sans-serif", position:"relative", overflowX:"hidden" }}>
      ...
      <header style={{ position:"sticky", top:0, zIndex:50, borderBottom:"1px solid rgba(0,255,135,0.08)", background:"rgba(6,9,16,0.85)", backdropFilter:"blur(20px)" }}>
        ...
        <div style={{ maxWidth:1140, margin:"0 auto", padding:"0 24px", display:"flex", gap:0, borderTop:"1px solid rgba(255,255,255,0.03)" }}>
          {[["assess","🩺 Risk Assessment"],["model","🤖 Model"],["stats","📊 Stats"]].map(([id,lbl]) => (
            <button key={id} onClick={() => setTab(id)} style={{ padding:"10px 22px", background:"none", border:"none", borderBottom:`2px solid ${tab===id?"#00ff87":"transparent"}`, color:tab===id?"#00ff87":"#475569", fontFamily:"'Outfit',sans-serif", fontSize:13, fontWeight:500, cursor:"pointer", transition:"all .2s" }}>{lbl}</button>
          ))}
        </div>
      </header>
          ...
          <div>
            ...
            <div className="fadeUp" style={{ textAlign:"center", paddingBottom:48, borderBottom:"1px solid rgba(255,255,255,0.04)", marginBottom:40 }}>
              ...
              <h1 style={{ fontFamily:"'Syne',sans-serif", fontSize:"clamp(2rem,4vw,3.2rem)", fontWeight:800, letterSpacing:"-1px", lineHeight:1.1, marginBottom:16 }}>
                <span style={{ background:"linear-gradient(135deg,#e2e8f0,#94a3b8)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Cardiovascular Risk</span>
                <br />
                <span style={{ background:"linear-gradient(135deg,#00ff87,#00c9ff)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Assessment System</span>
              </h1>
              <p style={{ color:"#475569", fontSize:16, maxWidth:480, margin:"0 auto", lineHeight:1.7 }}>17 clinical parameters · RuleNet hybrid model · Real-time risk stratification</p>
            ...
            <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:20, padding:"36px", backdropFilter:"blur(8px)", marginBottom:24 }}>
              ...
              <div key={step} style={{ animation:"fadeUp .35s ease" }}>
                {step===0 && (
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:20 }}>
                    <Num label="BMI" field="BMI" value={form.BMI} onChange={set} min="10" max="70" unit="kg/m²" />
                    <Num label="Physical Health (bad days/month)" field="PhysicalHealth" value={form.PhysicalHealth} onChange={set} min="0" max="30" step="1" unit="/30" />
                    <Num label="Mental Health (bad days/month)" field="MentalHealth" value={form.MentalHealth} onChange={set} min="0" max="30" step="1" unit="/30" />
                    <Num label="Sleep Time" field="SleepTime" value={form.SleepTime} onChange={set} min="0" max="24" step="0.5" unit="hrs" />
                  </div>
                )}
                {step===1 && (
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:20 }}>
                    <Toggle label="Do you smoke?" field="Smoking" value={form.Smoking} onChange={set} />
                    <Toggle label="Heavy Alcohol Drinking?" field="AlcoholDrinking" value={form.AlcoholDrinking} onChange={set} />
                    <Toggle label="Physically Active?" field="PhysicalActivity" value={form.PhysicalActivity} onChange={set} />
                    <Toggle label="Difficulty Walking?" field="DiffWalking" value={form.DiffWalking} onChange={set} />
                  </div>
                )}
                {step===2 && (
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:20 }}>
                ...
                {step===3 && (
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:20 }}>
                    ...
                    <Select label="General Health" field="GenHealth" value={form.GenHealth} options={GEN_H} onChange={set} />
                  </div>
                )}
              </div>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:36, paddingTop:24, borderTop:"1px solid rgba(255,255,255,0.05)" }}>
                <button onClick={prev} disabled={step===0} style={{ padding:"11px 28px", borderRadius:10, border:"1px solid rgba(255,255,255,0.08)", background:"rgba(255,255,255,0.03)", color:step===0?"#334155":"#94a3b8", fontSize:14, cursor:step===0?"default":"pointer", fontFamily:"'Outfit',sans-serif" }}>← Back</button>
                <div style={{ display:"flex", gap:8 }}>
                  {STEPS.map((_,i) => (
                    <div key={i} style={{ width:i===step?24:6, height:6, borderRadius:3, background:i<=step?"#00ff87":"rgba(255,255,255,0.08)", transition:"all .3s" }} />
                  ))}
                </div>
                {step < STEPS.length-1 ? (
                  <button onClick={next} style={{ padding:"11px 32px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#00ff87,#00c9ff)", color:"#060910", fontSize:14, fontWeight:700, cursor:"pointer", fontFamily:"'Syne',sans-serif", boxShadow:"0 0 24px rgba(0,255,135,0.25)" }}>Next →</button>
                ) : (
                  <button onClick={analyze} disabled={loading} style={{ padding:"11px 36px", borderRadius:10, border:loading?"1px solid rgba(0,255,135,0.3)":"none", background:loading?"rgba(0,255,135,0.1)":"linear-gradient(135deg,#00ff87,#00c9ff)", color:loading?"#00ff87":"#060910", fontSize:14, fontWeight:700, cursor:loading?"default":"pointer", fontFamily:"'Syne',sans-serif", display:"flex", alignItems:"center", gap:10, boxShadow:loading?"none":"0 0 32px rgba(0,255,135,0.3)" }}>
                    {loading ? (
                      <span style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <span style={{ width:16, height:16, border:"2px solid rgba(0,255,135,0.3)", borderTopColor:"#00ff87", borderRadius:"50%", animation:"spin .7s linear infinite", display:"inline-block" }} />
                        Analyzing...
                      </span>
                    ) : "Analyze Risk"}
                  </button>
            ...
            {result && (
              <div ref={resultRef} style={{ opacity:showRes?1:0, transform:showRes?"translateY(0)":"translateY(40px)", transition:"opacity .7s, transform .7s" }}>
                <div style={{ borderRadius:20, overflow:"hidden", marginBottom:24, border:`1px solid ${resultColor}22`, background:`linear-gradient(135deg,${resultColor}08,rgba(6,9,16,0.95) 70%)` }}>
                  ...
                  <div style={{ padding:"36px 40px", display:"flex", alignItems:"center", gap:40, flexWrap:"wrap" }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ color:"#64748b", fontSize:15 }}>Risk Level: <span style={{ color:resultColor, fontWeight:600 }}>{result.risk_level}</span><span style={{ margin:"0 12px", opacity:.3 }}>|</span>{result.risk_factors.length} risk factor{result.risk_factors.length!==1?"s":""} identified</div>
                    </div>
                    <RiskMeter pct={result.risk_percentage} level={result.risk_level} />
                  </div>
                </div>
                  ...
                  <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:16, padding:28 }}>
                    ...
                    {result.risk_factors.length===0 ? (
                      <div style={{ textAlign:"center", padding:"32px 0" }}>
                        ...
                        <div style={{ fontFamily:"'Syne',sans-serif", color:"#00ff87", fontWeight:600 }}>No Major Risk Factors</div>
                      </div>
                ...
                <div style={{ textAlign:"center", marginTop:28 }}>
                  <button onClick={() => { setResult(null); setShowRes(false); setStep(0); }} style={{ padding:"10px 28px", borderRadius:10, background:"none", border:"1px solid rgba(255,255,255,0.08)", color:"#64748b", fontSize:13, cursor:"pointer", fontFamily:"'Outfit',sans-serif" }}>← Start New Assessment</button>
                </div>
              </div>
            )}
          </div>
        ...
        {tab==="model" && (
          <div style={{ animation:"fadeUp .5s ease" }}>
            <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:20, padding:36, marginBottom:20 }}>
              ...
              <p style={{ color:"#94a3b8", lineHeight:1.8, marginBottom:28, maxWidth:640, fontFamily:"'Outfit',sans-serif" }}>RuleNet is a hybrid classifier combining domain-expert medical rules with a Random Forest ensemble. Rules provide high-confidence interpretable decisions; the RF handles ambiguous cases with probabilistic scoring.</p>
              <div style={{ display:"flex", gap:0, alignItems:"stretch", background:"rgba(0,0,0,0.3)", borderRadius:14, overflow:"hidden", border:"1px solid rgba(255,255,255,0.05)" }}>
                {[{ icon:"📥", title:"Input Layer", desc:"17 clinical features", color:"#00c9ff" },{ icon:"📏", title:"Rule Engine", desc:"3 medical rules\nHigh-confidence cases", color:"#a78bfa" },{ icon:"🌲", title:"Random Forest", desc:"500 estimators\nSMOTE balanced", color:"#fb923c" },{ icon:"🎯", title:"Output", desc:"Risk score + explanation", color:"#00ff87" }].map((s,i,arr) => (
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:14, marginBottom:20 }}>
              {[["94.21%","Test Accuracy","#00ff87"],["96.19%","Precision","#00c9ff"],["92.06%","Recall","#a78bfa"],["94.09%","F1-Score","#fb923c"],["0.98","AUC-ROC","#f472b6"],["94.23%","CV Accuracy","#facc15"]].map(([v,l,c]) => (
                <div key={l} style={{ background:"rgba(255,255,255,0.02)", border:`1px solid ${c}18`, borderRadius:14, padding:"22px 16px", textAlign:"center" }}>
                  <div style={{ fontFamily:"'Space Mono',monospace", fontSize:28, fontWeight:700, color:c }}>{v}</div>
                  <div style={{ color:"#94a3b8", fontSize:11, marginTop:6, fontFamily:"'Outfit',sans-serif" }}>{l}</div>
                </div>
              ))}
            </div>
            <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:16, padding:28 }}>
              <h3 style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:16, color:"#e2e8f0", marginBottom:20 }}>📊 Dataset and Pipeline</h3>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:10 }}>
                {[["Source","CDC BRFSS 2020"],["Total Records","319,795"],["Features","17"],["Class Balancing","SMOTE"],["Validation","5-Fold Stratified CV"],["Outlier Handling","IQR Capping"],["Encoding","Label Encoding"],["Missing Data","Mean Imputation"]].map(([k,v]) => (
          ...
          <div style={{ animation:"fadeUp .5s ease" }}>
            <div style={{ background:"linear-gradient(135deg,rgba(0,255,135,0.07),rgba(0,201,255,0.05))", border:"1px solid rgba(0,255,135,0.15)", borderRadius:16, padding:"22px 30px", marginBottom:22, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:14 }}>
              <div>
                ...
                <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:20, color:"#f1f5f9" }}>Model Performance Report</div>
                <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:13, color:"#64748b", marginTop:4 }}>Daffodil International University · FYDP 2025 · CDC BRFSS 2020</div>
              </div>
            ...
            <div style={{ background:"rgba(8,12,22,0.95)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:36, marginBottom:20 }}>
              ...
              <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:24, color:"#f1f5f9", marginBottom:6 }}>Model Comparison</h2>
              ...
              {[{ name:"RuleNet (Ours)", cv:94.23, test:94.21, highlight:true, tag:"★ BEST" },{ name:"Random Forest", cv:94.05, test:93.98 },{ name:"XGBoost", cv:93.80, test:98.10 },{ name:"KNN", cv:92.18, test:99.73 },{ name:"Naive Bayes", cv:91.54, test:91.55 },{ name:"Logistic Reg.", cv:91.50, test:91.33 },{ name:"SVM", cv:91.44, test:91.20 },{ name:"Decision Tree", cv:91.16, test:92.01 }].map((m,i) => (
                <div key={m.name} style={{ display:"flex", alignItems:"center", gap:14, marginBottom:13, opacity:0, animation:`fadeUp .35s ease ${i*0.07}s forwards` }}>
                  ...
                  <div style={{ width:150, flexShrink:0 }}>
                    <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:14, fontWeight:m.highlight?700:500, color:m.highlight?"#00ff87":"#cbd5e1", display:"flex", alignItems:"center", gap:6 }}>
                      {m.name}
                      {m.tag && <span style={{ fontSize:9, padding:"2px 7px", borderRadius:4, background:"rgba(0,255,135,0.12)", color:"#00ff87", border:"1px solid rgba(0,255,135,0.2)", fontFamily:"'Space Mono',monospace" }}>{m.tag}</span>}
                    </div>
                  </div>
                  <div style={{ flex:1, height:36, background:"rgba(255,255,255,0.04)", borderRadius:10, border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden", position:"relative" }}>
                    <div style={{ position:"absolute", top:0, bottom:0, left:0, width:`${((m.cv-88)/10)*100}%`, background:m.highlight?"linear-gradient(90deg,#00c864,#00ff87,#00deff)":"linear-gradient(90deg,rgba(71,85,105,0.7),rgba(51,65,85,0.5))", borderRadius:10, boxShadow:m.highlight?"0 0 18px rgba(0,255,135,0.25)":"none", transition:"width 1.3s cubic-bezier(.34,1.2,.64,1)" }} />
                  ...
                  <div style={{ width:88, flexShrink:0, textAlign:"right" }}>
                    <div style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:"#475569" }}>test</div>
            ...
            <div style={{ background:"rgba(8,12,22,0.95)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:36, marginBottom:20 }}>
              <div style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:"#a78bfa", letterSpacing:3, textTransform:"uppercase", marginBottom:6 }}>RANDOM FOREST ANALYSIS</div>
              <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:24, color:"#f1f5f9", marginBottom:4 }}>Feature Importance</h2>
              <p style={{ color:"#64748b", fontSize:13, marginBottom:28, fontFamily:"'Outfit',sans-serif" }}>Top 10 cardiovascular risk predictors by importance score</p>
              {[{ name:"Age Category", pct:14.2, col:"#f43f5e", icon:"👴" },{ name:"BMI", pct:13.8, col:"#fb923c", icon:"⚖️" },{ name:"General Health", pct:12.5, col:"#facc15", icon:"💚" },{ name:"Physical Health", pct:11.7, col:"#84cc16", icon:"🏃" },{ name:"Mental Health", pct:8.9, col:"#00ff87", icon:"🧠" },{ name:"Diabetic Status", pct:7.8, col:"#00c9ff", icon:"💉" },{ name:"Sleep Time", pct:6.4, col:"#818cf8", icon:"😴" },{ name:"Diff. Walking", pct:5.9, col:"#a78bfa", icon:"🚶" },{ name:"Stroke History", pct:5.2, col:"#f472b6", icon:"🧬" },{ name:"Kidney Disease", pct:4.8, col:"#fb7185", icon:"🫘" }].map((f,i) => (
                <div key={f.name} style={{ display:"flex", alignItems:"center", gap:14, marginBottom:13, opacity:0, animation:`fadeUp .35s ease ${i*0.06}s forwards` }}>
                  <div style={{ width:34, height:34, borderRadius:9, display:"flex", alignItems:"center", justifyContent:"center", background:`${f.col}15`, border:`1px solid ${f.col}30`, fontSize:15, flexShrink:0 }}>{f.icon}</div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:14 }}>
              {[{ label:"Dataset Year", value:"2020", sub:"CDC BRFSS Source", col:"#00c9ff", icon:"📅" },{ label:"Training Records", value:"319,795", sub:"Post-SMOTE Balance", col:"#00ff87", icon:"🗄️" },{ label:"Test Split", value:"80 / 20", sub:"Stratified Split", col:"#a78bfa", icon:"✂️" },{ label:"CV Strategy", value:"5-Fold", sub:"StratifiedKFold", col:"#fb923c", icon:"🔄" },{ label:"Best Algorithm", value:"RuleNet", sub:"Hybrid ML + Rules", col:"#f43f5e", icon:"🏆" },{ label:"Report Year", value:"2025", sub:"FYDP Submission", col:"#facc15", icon:"🎓" }].map(m => (
      ...
      <footer style={{ borderTop:"1px solid rgba(0,255,135,0.06)", padding:"22px 24px", position:"relative", zIndex:2, background:"rgba(6,9,16,0.9)", backdropFilter:"blur(10px)" }}>
        <div style={{ maxWidth:1140, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
          <div style={{ fontFamily:"'Space Mono',monospace", fontSize:12, fontWeight:700, color:"#00ff87", letterSpacing:1 }}>HEARTGUARD AI</div>
          ...
          <div style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:"#334155", letterSpacing:1 }}>EDUCATIONAL USE ONLY</div>
        </div>
      </footer>
    </div>
  );
