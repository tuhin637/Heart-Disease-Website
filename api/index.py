...
def analyze_risk_factors(data: PredictionRequest) -> list[RiskFactor]:
    factors = []
    if data.BMI >= 35:
        factors.append(RiskFactor(factor="Severe Obesity", value=f"BMI {data.BMI:.1f}", impact="high",
            advice="Target BMI below 25 through structured diet and exercise with specialist guidance."))
    elif data.BMI >= 30:
        factors.append(RiskFactor(factor="Obesity", value=f"BMI {data.BMI:.1f}", impact="medium",
            advice="Aim for gradual weight loss of 0.5-1 kg/week through lifestyle changes."))
    if data.Smoking == "Yes":
        factors.append(RiskFactor(factor="Active Smoking", value="Yes", impact="high",
            advice="Enroll in a cessation program. Consider nicotine replacement therapy."))
    if data.Stroke == "Yes":
        factors.append(RiskFactor(factor="History of Stroke", value="Yes", impact="high",
            advice="Regular cardiac monitoring and cardiologist consultation required."))
    if data.PhysicalHealth > 15:
        factors.append(RiskFactor(factor="Poor Physical Health",
            value=f"{int(data.PhysicalHealth)} bad days/month", impact="high",
            advice="Seek comprehensive medical evaluation for underlying chronic conditions."))
    elif data.PhysicalHealth > 7:
        factors.append(RiskFactor(factor="Declining Physical Health",
            value=f"{int(data.PhysicalHealth)} bad days/month", impact="medium",
            advice="Monitor symptoms and discuss with your physician."))
    age_idx = AGE_CATEGORIES.index(data.AgeCategory)
    if age_idx >= 10:
        factors.append(RiskFactor(factor="Advanced Age (70+)", value=data.AgeCategory, impact="high",
            advice="Annual cardiac checkups strongly recommended."))
    elif age_idx >= 8:
        factors.append(RiskFactor(factor="Age 60-69", value=data.AgeCategory, impact="medium",
            advice="Regular health screenings every 6 months advised."))
    if data.Diabetic in ["Yes","Yes (during pregnancy)"]:
        factors.append(RiskFactor(factor="Diabetes", value=data.Diabetic, impact="high",
            advice="Strict blood sugar control. Regular HbA1c monitoring is essential."))
    elif data.Diabetic == "No, borderline diabetes":
        factors.append(RiskFactor(factor="Borderline Diabetes", value=data.Diabetic, impact="medium",
            advice="Diet modification and regular glucose monitoring recommended."))
    if data.PhysicalActivity == "No":
        factors.append(RiskFactor(factor="Sedentary Lifestyle", value="No regular exercise", impact="medium",
            advice="Aim for 150 minutes/week of moderate aerobic activity."))
    if data.KidneyDisease == "Yes":
        factors.append(RiskFactor(factor="Kidney Disease", value="Yes", impact="high",
            advice="CKD significantly elevates cardiovascular risk. Specialist consultation needed."))
    if data.SleepTime < 6 or data.SleepTime > 9:
        factors.append(RiskFactor(factor="Poor Sleep Duration",
            value=f"{data.SleepTime:.0f} hrs/night", impact="low",
            advice="Target 7-9 hours nightly. Evaluate for sleep apnea if needed."))
    if data.AlcoholDrinking == "Yes":
        factors.append(RiskFactor(factor="Heavy Alcohol Consumption", value="Yes", impact="medium",
            advice="Reduce to 2 or fewer drinks/day (men) or 1 (women)."))
    return factors
...
def predict(request: PredictionRequest):
    ...
    if model is None:
        raise HTTPException(503, "Model not loaded. Please run train_model.py first.")
    try:
