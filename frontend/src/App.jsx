    ...
    const draw = () => {
      ...
      for (let i=0;i<pts.length;i++) for (let j=i+1;j<pts.length;j++) {
        ...
        if (d<100) {
          ...
          ctx.strokeStyle = `rgba(0,255,135,${(1-d/100)*0.08})`;
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      }
    ...
    <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:32 }}>
      {STEPS.map((s,i) => (
        <div key={i} style={{ display:"flex", alignItems:"center", flex:1 }}>
          ...
          <div style={{ marginLeft:8, display:current!==i&&"none"||"block" }}>
          ...
          {i < total-1 && (
            <div style={{ flex:1, height:1, marginLeft:8,
              background: i < current-1 ? "linear-gradient(90deg,#00ff87,#00c9ff)"
                        : i === current-1 ? `linear-gradient(90deg,#00ff87,rgba(255,255,255,0.06))`
        ...
        <button key={opt} type="button"
          ...
          style={{
            ...
            background: value===opt
              ? (opt==="Yes"?"linear-gradient(135deg,#ef4444,#dc2626)":"linear-gradient(135deg,#00ff87,#00c9ff)")
              : "rgba(255,255,255,0.03)",
            color: value===opt ? (opt==="Yes"?"#fff":"#0a0a0a") : "#64748b",
            transition:"all .25s",
          }}>{opt}</button>
      ))}
    </div>
...
export default function HeartGuardPremium() {
  ...
  // Simulated BPM counter
  useEffect(() => {
    const t = setInterval(() => setBpm(b => 68 + Math.floor(Math.random()*12)), 800);
  ...
  useEffect(() => {
    ...
    style.textContent = `
      *{box-sizing:border-box;margin:0;padding:0;}
      ...
      @keyframes glitch{0%,100%{clip-path:inset(0 0 95% 0)}20%{clip-path:inset(40% 0 50% 0)}40%{clip-path:inset(80% 0 5% 0)}}
    ...
    <div style={{ minHeight:"100vh", background:"#060910", color:"#e2e8f0", fontFamily:"'Outfit',sans-serif", position:"relative", overflowX:"hidden" }}>
      ...
      <header style={{ position:"sticky", top:0, zIndex:50, borderBottom:"1px solid rgba(0,255,135,0.08)", background:"rgba(6,9,16,0.85)", backdropFilter:"blur(20px)" }}>
        <div style={{ maxWidth:1140, margin:"0 auto", padding:"0 24px", height:64, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:14 }}>
            <div>
              <div style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:15, letterSpacing:"-0.3px", background:"linear-gradient(90deg,#00ff87,#00c9ff)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", lineHeight:1.3, maxWidth:480 }}>Predicting Heart Disease Using Machine Learning</div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:20 }}>
            ...
            <div style={{ display:"flex", gap:6 }}>
              ...
              <span style={{ fontSize:10, padding:"4px 10px", borderRadius:20, background:"rgba(0,201,255,0.08)", border:"1px solid rgba(0,201,255,0.15)", color:"#00c9ff", fontFamily:"'Space Mono',monospace" }}>2024</span>
            </div>
          </div>
        </div>
      ...
      <main style={{ maxWidth:1140, margin:"0 auto", padding:"40px 24px 80px", position:"relative", zIndex:2 }}>
        ...
        {tab==="assess" && (<>
          ...
          <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:20, padding:"36px", backdropFilter:"blur(8px)", marginBottom:24 }}>
            ...
            {/* ECG divider */}
            <div style={{ marginBottom:28, opacity:.5 }}>
              <ECGLine active={true} color="#00ff8730"/>
            </div>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:36, paddingTop:24, borderTop:"1px solid rgba(255,255,255,0.05)" }}>
              ...
              {step < STEPS.length-1 ? (
                <button onClick={next} style={{
                  ...
                  fontFamily:"'Syne',sans-serif", letterSpacing:.5, boxShadow:"0 0 24px rgba(0,255,135,0.25)",
                  transition:"all .2s",
                }}>Next →</button>
              ) : (
                <button onClick={analyze} disabled={loading} style={{
                  ...
                  boxShadow: loading?"none":"0 0 32px rgba(0,255,135,0.3)",
                  display:"flex", alignItems:"center", gap:10, transition:"all .3s",
                }}>
                  {loading ? (
                    <>
                      <div style={{ width:16, height:16, border:"2px solid rgba(0,255,135,0.3)", borderTopColor:"#00ff87", borderRadius:"50%", animation:"spin .7s linear infinite" }}/>
                      Analyzing...
                    </>
                  ) : <>Analyze Risk</>}
                </button>
          ...
          {result && (
            <div ref={resultRef} style={{ opacity:showRes?1:0, transform:showRes?"translateY(0)":"translateY(40px)", transition:"opacity .7s, transform .7s" }}>
              ...
              <div style={{
                ...
                background:`linear-gradient(135deg, ${resultColor}08 0%, rgba(6,9,16,0.95) 70%)`,
              }}>
                {/* Animated top bar */}
                ...
                <div style={{ padding:"36px 40px", display:"flex", alignItems:"center", gap:40, flexWrap:"wrap" }}>
                  ...
                  {/* Text */}
                  <div style={{ flex:1 }}>
                    <div style={{ fontFamily:"'Space Mono',monospace", fontSize:11, color:"#475569", letterSpacing:3, textTransform:"uppercase", marginBottom:8 }}>
                      PREDICTION COMPLETE · {result.model_used}
                    </div>
                    <div style={{ fontFamily:"'Syne',sans-serif", fontSize:36, fontWeight:800, color:resultColor, letterSpacing:"-0.5px", lineHeight:1, marginBottom:8 }}>
                      {result.risk_label}
                    </div>
              ...
              <div style={{ textAlign:"center", marginTop:28 }}>
                <button onClick={()=>{ setResult(null); setShowRes(false); setStep(0); }} style={{ padding:"10px 28px", borderRadius:10, background:"none", border:"1px solid rgba(255,255,255,0.08)", color:"#64748b", fontSize:13, cursor:"pointer", fontFamily:"'Outfit',sans-serif", transition:"all .2s" }}>
              ...
              <div style={{ display:"flex", gap:0, alignItems:"stretch", background:"rgba(0,0,0,0.3)", borderRadius:14, overflow:"hidden", border:"1px solid rgba(255,255,255,0.05)" }}>
                {[
                  ...
                  <div key={i} style={{ flex:1, padding:"24px 20px", position:"relative", borderRight: i<arr.length-1?"1px solid rgba(255,255,255,0.05)":"none" }}>
                    ...
                    {i<arr.length-1 && (
                      <div style={{ position:"absolute", right:-14, top:"50%", transform:"translateY(-50%)", color:"#334155", fontSize:20, zIndex:1 }}>→</div>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div style={{ background:"rgba(8,12,22,0.95)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:20, padding:36, marginBottom:20 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:30, flexWrap:"wrap", gap:12 }}>
                <div>
                  ...
                  <h2 style={{ fontFamily:"'Syne',sans-serif", fontWeight:800, fontSize:24, color:"#f1f5f9", margin:0 }}>Model Comparison</h2>
                  <p style={{ color:"#64748b", fontSize:13, marginTop:6, fontFamily:"'Outfit',sans-serif" }}>5-Fold Stratified Cross-Validation · 319,795 records · SMOTE balanced</p>
                </div>
                <div style={{ display:"flex", gap:16, fontSize:12, fontFamily:"'Space Mono',monospace" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                    <div style={{ width:14, height:10, borderRadius:3, background:"linear-gradient(90deg,#00ff87,#00c9ff)" }}/>
                    <span style={{ color:"#94a3b8" }}>CV Score</span>
                  </div>
                  <div style={{ display:"flex", alignItems:"center", gap:7 }}>
                    <div style={{ width:14, height:10, borderRadius:3, background:"rgba(100,116,139,0.5)", border:"1px solid rgba(100,116,139,0.4)" }}/>
                    <span style={{ color:"#94a3b8" }}>Test Score</span>
                  </div>
                </div>
              </div>
              ...
              {[
                ...
                return (
                  <div key={m.name} style={{ display:"flex", alignItems:"center", gap:14, marginBottom:13, opacity:0, animation:`fadeUp .35s ease ${i*0.07}s forwards` }}>
                    {/* Rank badge */}
                    <div style={{ width:28, height:28, borderRadius:8, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", background: m.highlight?"linear-gradient(135deg,#00ff87,#00c9ff)":"rgba(255,255,255,0.06)", fontFamily:"'Space Mono',monospace", fontSize:11, fontWeight:700, color: m.highlight?"#060910":"#64748b" }}>
                      {i+1}
                    </div>
                    {/* Name */}
                    ...
                    <div style={{ flex:1, height:36, background:"rgba(255,255,255,0.04)", borderRadius:10, border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden", position:"relative" }}>
                      <div style={{
                        ...
                        background: m.highlight
                          ? "linear-gradient(90deg,#00c864,#00ff87,#00deff)"
                          : "linear-gradient(90deg,rgba(71,85,105,0.7),rgba(51,65,85,0.5))",
                        borderRadius:10,
                        boxShadow: m.highlight ? "0 0 18px rgba(0,255,135,0.25), inset 0 1px 0 rgba(255,255,255,0.15)" : "none",
                        transition:"width 1.3s cubic-bezier(.34,1.2,.64,1)",
                      }}/>
                      <div style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", fontFamily:"'Space Mono',monospace", fontSize:14, fontWeight:700, color: m.highlight?"#001a0a":"#e2e8f0", zIndex:1, textShadow: m.highlight?"none":"0 1px 3px rgba(0,0,0,0.5)" }}>
                        {m.cv}%
                      </div>
                    </div>
                    {/* Test value */}
              ...
              <div style={{ marginTop:22, padding:"14px 18px", background:"rgba(249,115,22,0.06)", border:"1px solid rgba(249,115,22,0.18)", borderRadius:10, display:"flex", gap:12, alignItems:"flex-start" }}>
                ...
                <p style={{ fontFamily:"'Outfit',sans-serif", fontSize:13, color:"#94a3b8", margin:0, lineHeight:1.7 }}>
                  KNN & XGBoost show inflated test accuracy due to memorization on training data. <strong style={{ color:"#e2e8f0" }}>RuleNet achieves the highest cross-validation accuracy (94.23%)</strong>, proving superior real-world generalization — the key metric for clinical AI systems.
                </p>
              </div>
            </div>
                ...
                <div key={f.name} style={{ display:"flex", alignItems:"center", gap:14, marginBottom:13, opacity:0, animation:`fadeUp .35s ease ${i*0.06}s forwards` }}>
                  ...
                  <div style={{ flex:1, height:30, background:"rgba(255,255,255,0.04)", borderRadius:8, border:"1px solid rgba(255,255,255,0.06)", overflow:"hidden", position:"relative" }}>
                    <div style={{
                      position:"absolute", top:0, bottom:0, left:0,
                      width:`${(f.pct/15)*100}%`,
                      background:`linear-gradient(90deg,${f.col}88,${f.col})`,
                      borderRadius:8,
                      boxShadow:`0 0 10px ${f.col}40`,
                      transition:"width 1.1s cubic-bezier(.34,1.2,.64,1)",
                    }}/>
                    <div style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", fontFamily:"'Space Mono',monospace", fontSize:12, fontWeight:700, color:"#ffffff", zIndex:1 }}>
                      {f.pct}%
                    </div>
                  </div>
                  {/* Rank */}
      ...
      <footer style={{ borderTop:"1px solid rgba(0,255,135,0.06)", padding:"22px 24px", position:"relative", zIndex:2, background:"rgba(6,9,16,0.9)", backdropFilter:"blur(10px)" }}>
        <div style={{ maxWidth:1140, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
          ...
          <div style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:"#334155", letterSpacing:1 }}>
